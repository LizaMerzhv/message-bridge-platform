...
class NotificationAdminControllerTest {
  ...
  void list_ShouldApplyFiltersAndReturnResults() throws Exception {
    ...
    when(notificationService.findAll(any(NotificationFilter.class), any(PageRequest.class)))
        .thenReturn(page);
    ...
    ArgumentCaptor<NotificationFilter> filterCaptor = ArgumentCaptor.forClass(NotificationFilter.class);
  ...
  void getById_ShouldReturnDetailWithDeliveries() throws Exception {
    ...
    when(notificationService.findById(view.getId())).thenReturn(view);
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.deliveries[0].timestamp").value("2024-01-01T00:00:05Z"));
  }
  ...
  void create_ShouldReturnCreatedNotification() throws Exception {
    ...
    client.setRateLimitPerMin(30);
    when(clientRepository.findById(clientId)).thenReturn(Optional.of(client));

    var entity = new com.example.notifi.api.data.entity.NotificationEntity();
    entity.setId(UUID.randomUUID());
    entity.setStatus(NotificationStatus.CREATED);
    when(notificationService.create(any(), any())).thenReturn(new CreateNotificationResult(entity, false));
    when(notificationService.findById(entity.getId())).thenReturn(view());

    AdminCreateNotificationRequest request = new AdminCreateNotificationRequest();
    request.setClientId(clientId);
    request.setChannel(com.example.notifi.common.model.Channel.EMAIL);
    request.setTo("user@example.com");
    request.setSubject("Admin subject");
    request.setExternalRequestId("ext-9");
    ...
    mockMvc
        ...
        .andExpect(header().string("Location", "/admin/notifications/" + entity.getId()))
