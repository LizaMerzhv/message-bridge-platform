...
class TaskConsumerTest {
    ...
    void failureRetriesWhenAttemptsRemain() throws Exception {
        ...
        when(retryPolicy.evaluate(1))
            .thenReturn(
                RetryDecision.retry(2, java.time.Duration.ofSeconds(8), java.time.Duration.ZERO));
        ...
        assertThat(metrics.registry().get("deliveries_failed_total").counter().count())
            .isEqualTo(1.0d);

        verify(notificationApiClient, never())
            .sendDeliveryResult(any(), any(), anyInt(), any(), any(), any());
    }
