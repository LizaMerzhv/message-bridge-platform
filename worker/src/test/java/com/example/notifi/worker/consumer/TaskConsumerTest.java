...
class TaskConsumerTest {
  ...
  void successMarksNotificationAndDelivery() throws Exception {
    ...
    NotificationMessage message = createMessage(notification.getId(), 1);
  ...
  void failureRetriesWhenAttemptsRemain() throws Exception {
    ...
    verify(amqpPublisher)
        .publishRetry(message.withAttempt(2), java.time.Duration.ofSeconds(8), java.time.Duration.ZERO);
  ...
  void failureRoutesToDlqAfterMaxAttempts() throws Exception {
    ...
    NotificationMessage message = createMessage(notification.getId(), 3);
    consumer.handle(message);
  ...
  private NotificationMessage createMessage(UUID notificationId, int attempt) {
