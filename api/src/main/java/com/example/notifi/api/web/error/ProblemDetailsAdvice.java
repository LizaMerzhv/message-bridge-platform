...
public class ProblemDetailsAdvice {
  ...
  public ResponseEntity<ProblemDetails> handleValidationErrors(
  ...
  public ResponseEntity<ProblemDetails> handleConstraintViolation(
    ...
    ProblemDetails body =
        Problems.badRequest(
            "Constraint violation: " + ex.getMessage(), req.getRequestURI(), traceId());
    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        .contentType(APPLICATION_PROBLEM_JSON)
        .body(body);
  }
  ...
  public ResponseEntity<ProblemDetails> handleUnreadable(
    ...
    return ResponseEntity.badRequest().contentType(APPLICATION_PROBLEM_JSON).body(body);
  }
  ...
  public ResponseEntity<ProblemDetails> handleMissingParam(
    ...
    ProblemDetails body =
        Problems.badRequest(
            "Missing parameter: " + ex.getParameterName(), req.getRequestURI(), traceId());
    return ResponseEntity.badRequest().contentType(APPLICATION_PROBLEM_JSON).body(body);
  }

  @ExceptionHandler(NotificationNotFoundException.class)
  public ResponseEntity<ProblemDetails> handleNotificationNotFound(
  ...
  public ResponseEntity<ProblemDetails> handleTemplateNotFound(
  ...
  public ResponseEntity<ProblemDetails> handleTemplateCodeNotFound(
      TemplateCodeNotFoundException ex, HttpServletRequest req) {
    ProblemDetails body = Problems.notFound(ex.getMessage(), req.getRequestURI(), traceId());

    return ResponseEntity.status(404).contentType(APPLICATION_PROBLEM_JSON).body(body);
  }

  @ExceptionHandler(TemplateInactiveException.class)
  public ResponseEntity<ProblemDetails> handleTemplateInactive(
      TemplateInactiveException ex, HttpServletRequest req) {
    ProblemDetails body =
        Problems.of(
            422,
            ...
            null,
            null);
    return ResponseEntity.unprocessableEntity().contentType(APPLICATION_PROBLEM_JSON).body(body);
  }

  @ExceptionHandler(SendAtWindowException.class)
  public ResponseEntity<ProblemDetails> handleSendAt(
      SendAtWindowException ex, HttpServletRequest req) {
    ProblemDetails body =
        Problems.of(
            422,
  ...
  public ResponseEntity<ProblemDetails> handleConflict(
    ...
    String detail =
        ex.getMostSpecificCause() != null ? ex.getMostSpecificCause().getMessage() : "Conflict";
