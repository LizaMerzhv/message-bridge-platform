...
class RateLimitInterceptorTest extends WebTestBase {
    ...
    void should_pass_when_allowed() throws Exception {
        ...
        var principal = new ClientPrincipal(
            UUID.fromString("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"), "Test", 10);

        var result = mvc.perform(
            get("/ping").requestAttr(ClientPrincipal.class.getName(), principal)
        ).andReturn();
    ...
    void should_block_with_429_problem_when_exceeded() throws Exception {
        ...
        var principal = new ClientPrincipal(
            UUID.fromString("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"), "Test", 10);
        ...
        ProblemDetails body = om.readValue(result.getResponse().getContentAsByteArray(), ProblemDetails.class);
